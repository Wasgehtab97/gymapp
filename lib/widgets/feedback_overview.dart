...
class _FeedbackOverviewState extends State<FeedbackOverview> {
  ...
  Future<void> _fetchFeedbacks() async {
    ...
    try {
      ...
      final response =
          await http.get(Uri.parse('$API_URL/api/feedback$queryString'));
      final result = jsonDecode(response.body);
      if (response.statusCode == 200 && result['data'] != null) {
        setState(() {
          ...
          Row(
            children: [
              ...
              DropdownButton<String>(
                ...
                items: [
                  const DropdownMenuItem(value: '', child: Text("Alle Geräte")),
              ...
              DropdownButton<String>(
                ...
                items: const [
                  ...
                  DropdownMenuItem(value: 'neu', child: Text("Neu")),
                  DropdownMenuItem(
                  ...
                  child: DataTable(
                    ...
                    rows: feedbacks.map<DataRow>((fb) {
                      ...
                      final createdAt = DateTime.tryParse(
                              fb['created_at']?.toString() ?? '')?.toLocal() ??
                          DateTime.now();
                      final formattedDate =
                          "${createdAt.day.toString().padLeft(2, '0')}.${createdAt.month.toString().padLeft(2, '0')}.${createdAt.year}";
                      return DataRow(
                        cells: [
                          DataCell(Text(fb['id'].toString())),
                          DataCell(Text(deviceName)),
                          DataCell(Text(fb['feedback_text'].toString())),
                          DataCell(Text(formattedDate)),
                          DataCell(Text(fb['status'].toString())),
                          DataCell(
                            fb['status'] != 'erledigt'
                                ? ElevatedButton(
                                    onPressed: () =>
                                        _updateFeedbackStatus(fb['id'], 'erledigt'),
